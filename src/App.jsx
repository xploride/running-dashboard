...
function HomeView({ runs, routes, onAdd, onRoute }) {
  ...
  return <div className="view home-view">
    <section className="home-hero">
      <div className="home-hero__top"><p className="eyebrow">THIS WEEK</p><span className="streak"><Flame size={15} fill="currentColor"/> 7 DAY FLOW</span></div>
    ...
    <section className="section-block">
      <div className="section-heading"><div><p className="eyebrow">LATEST EFFORTS</p><h2>Recent runs</h2></div><span>{runs.length} TOTAL</span></div>
      {recent.length ? <div className="recent-list">{recent.map((run) => <button key={run.id} className="recent-run" onClick={() => routes[run.date] && onRoute(run.date)}><RouteSketch coords={routes[run.date]?.coords}/><div className="recent-run__body"><span>{formatDate(run.date, { month:'long',day:'numeric',weekday:'short' })}</span><strong>{run.distance.toFixed(2)} KM</strong><small>{paceText(run.pace)} /KM · {durationText(run.duration)}</small></div><ChevronRight size={20}/></button>)}</div> : <EmptyState title="첫 러닝을 기록하세요" text="수동 입력하거나 GPX와 Apple 건강 기록을 가져올 수 있습니다."/>}
    </section>
  </div>
}
...
function RunsView({ runs, routes, onAdd, onDelete, onRoute, onAttachRoute, nativeHealth, onHealthSave }) {
  ...
  return <div className="view">
    <header className="view-title"><div><p className="eyebrow">TRAINING LOG</p><h1>Every run<br/>counts.</h1></div><button className="round-action" onClick={onAdd} aria-label="러닝 추가"><CirclePlus/></button></header>
...
function InsightsView({ runs }) {
  ...
  return <div className="view">
    <header className="view-title"><div><p className="eyebrow">TRAIN SMARTER</p><h1>See the<br/>pattern.</h1></div><Sparkles className="title-icon"/></header>
...
function SettingsView({ onHealthImport, onGpxImport, syncing, nativeHealth, onHealthAuthorize }) {
  ...
  return <div className="view">
    <header className="view-title"><div><p className="eyebrow">CONTROL CENTER</p><h1>Bring your<br/>run with you.</h1></div></header>
    <section className="settings-list"><button onClick={()=>healthRef.current?.click()}><span className="settings-icon"><HeartPulse/></span><span><strong>Apple 건강 가져오기</strong><small>export.xml · 러닝 요약만 저장</small></span><Upload/></button><input ref={healthRef} hidden type="file" accept=".xml,text/xml,application/xml" onChange={onHealthImport}/><button onClick={()=>gpxRef.current?.click()}><span className="settings-icon"><Route/></span><span><strong>GPX 경로 연결</strong><small>좌표는 현재 브라우저 세션에만 보관</small></span><Upload/></button><input ref={gpxRef} hidden type="file" accept=".gpx,application/gpx+xml" onChange={onGpxImport}/></section>
...
export default function App() {
  ...
  const [activeRouteDate,setActiveRouteDate]=useState(null),[routeVersion,setRouteVersion]=useState(0),[autoPlay,setAutoPlay]=useState(false)
  ...
  const attachInputRef=useRef(null),attachRunRef=useRef(null)
  ...
  const importGpxFile=async(file,expectedRun=null)=>{const parsed=parseGpx(await readFile(file),file.name);if(expectedRun&&parsed.run.date!==expectedRun.date)throw new Error(`GPX 날짜(${parsed.run.date})가 기록 날짜(${expectedRun.date})와 다릅니다.`);const matchedRun=expectedRun||parsed.run;storeRoute(matchedRun,parsed.coords);if(!expectedRun){const merged=mergeRuns(runs,[parsed.run]);if(merged.added)await persist(merged.runs)}setError('');setRouteVersion((value)=>value+1);setAutoPlay(Boolean(expectedRun));setTab('map');notify(expectedRun?'경로를 연결했습니다.':'GPX 기록과 경로를 불러왔습니다.')}
  ...
  return <div className="app-shell">
    ...
    <div className="app-main"><header className="topbar"><div className="mobile-brand">RUNNING<span>/03</span></div><div className="topbar-status"><span className={saving?'syncing':''}>{saving?'SYNCING':'ALL MOVES SAVED'}</span><button className="icon-button" onClick={load} aria-label="새로고침"><RefreshCw size={18}/></button></div></header>{error&&<div className="error-banner"><span>{error}</span><button onClick={()=>setError('')}><X size={16}/></button></div>}<main>{loading?<div className="loading-state"><LoaderCircle className="spin"/><span>LOADING YOUR RUNS</span></div>:<>{tab==='home'&&<HomeView runs={runs} routes={routes} onAdd={()=>setShowAdd(true)} onRoute={showRoute}/>} {tab==='log'&&<RunsView runs={runs} routes={routes} onAdd={()=>setShowAdd(true)} onDelete={removeRun} onRoute={showRoute} onAttachRoute={attachRoute} nativeHealth={nativeHealth} onHealthSave={saveHealthRun}/>} {tab==='map'&&<div className="map-view"><Suspense fallback={<div className="loading-state"><LoaderCircle className="spin"/><span>LOADING MAP</span></div>}><RunMap route={activeRoute} autoPlay={autoPlay}/></Suspense><div className="route-strip">{Object.values(routes).map((route)=><button key={route.id} className={activeRouteDate===route.run.date?'active':''} onClick={()=>{setActiveRouteDate(route.run.date);setAutoPlay(false);setRouteVersion((value)=>value+1)}}><span>{formatDate(route.run.date)}</span><strong>{route.run.distance.toFixed(1)} KM</strong></button>)}</div></div>} {tab==='insights'&&<InsightsView runs={runs}/>} {tab==='settings'&&<SettingsView onHealthImport={handleHealthImport} onGpxImport={handleGpxImport} syncing={saving} nativeHealth={nativeHealth} onHealthAuthorize={authorizeHealth}/>}</>}</main></div>
    ...
    <input ref={attachInputRef} hidden type="file" accept=".gpx,application/gpx+xml" onChange={handleAttach}/>{showAdd&&<AddRunSheet onClose={()=>setShowAdd(false)} onSave={saveRun}/>} {celebration&&<Celebration run={celebration} onClose={()=>setCelebration(null)}/>} {toast&&<div className="toast"><MessageCircle size={16}/>{toast}</div>}
  </div>
}
